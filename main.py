...
class Instructor():
    ...
    def prepare(self):
        ...
        self.args.log_path = args.log_path + args.dataset + " " + str(args.ent_r) + " " + str(args.num_ent_layers) + " " + str(args.learning_rate) + " " + str(args.batch_size) +"/"
        if not os.path.exists(args.log_path):
            os.mkdir(args.log_path)
        self.args.log_path = args.log_path + args.dataset + " " + datetime.now().strftime("%Y%m%d%H%M%S")
        if self.args.note != "":
    ...
    def train(self):
        ...
        for epoch in range(int(self.args.epoch_num)):
            ...
            if valid_res[self.args.valid_metrics] > self.best_valid:
                ...
                if self.stop_epoch >= int(self.args.patience):
                    self.args.logger.info(
            ...
            if epoch % 1 == 0:
                self.args.logger.info(
                    f"Snapshot:{self.args.snapshot}\t{self.stop_epoch}\t{self.args.patience}\tEpoch:{epoch}\tLoss:{round(loss, 3)}\tMRR:{round(valid_res['mrr'] * 100, 3)}\tHits@10:{round(valid_res['hits10'] * 100, 3)}\tBest:{round(self.best_valid * 100, 3)}"
                )
        end_time = time.time()
